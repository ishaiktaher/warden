...
def load_settings() -> Settings:
    ...
    if environment == "prod" and not admin_key:
        raise RuntimeError("CONTROL_PLANE_ADMIN_KEY is required for break-glass administration")
    if not admin_key:
    ...
    if environment == "prod":
        missing = [
            name for name, value in (
                ...
                ("REDIS_URL", redis_url),
                ("WARDEN_SIGNING_PROVIDER", signing_provider if signing_provider != "local" else None),
                ("WARDEN_SECRETS_PROVIDER", secrets_provider if secrets_provider != "local" else None),
                ("WARDEN_AUDIT_PROVIDER", audit_provider if audit_provider != "local" else None),
            ) if not value
        ]
        if missing:
            raise RuntimeError(
    ...
    public_url = os.getenv("WARDEN_PUBLIC_URL", "http://127.0.0.1:8000").strip().rstrip("/")
    ...
    return Settings(
        ...
        oidc_on_behalf_of_claim=os.getenv("WARDEN_OIDC_ON_BEHALF_OF_CLAIM", "on_behalf_of"),
        ...
        signing_provider_url=os.getenv("WARDEN_SIGNING_PROVIDER_URL", "").strip() or None,
        secrets_provider_url=os.getenv("WARDEN_SECRETS_PROVIDER_URL", "").strip() or None,
