...
from .identity import AppIdentityService
...
class ControlPlane:
    TRACE_STAGES = (
        (10.0, "capability_check"),
        (20.0, "grant_check"),
        (30.0, "policy_layers"),
        (40.0, "approval_gate"),
        (50.0, "credential_resolution"),
        (60.0, "connector_dispatch"),
    )
    ...
    def __init__(self, settings: Settings | None = None):
        ...
        self.api_keys = APIKeyService(self.database, self.audit)
        self.connect_sessions = ConnectSessionService(
            self.database, self.settings, self.audit
        )
        ...
        self.identity = AppIdentityService(
            self.database,
            self.settings,
            self.secrets,
    ...
    def create_owner(
        self, owner_id: str, name: str, roles: list[str], actor: str
    ) -> dict[str, Any]:
        ...
        self.audit.append(
            "owner.registered", actor, payload={"owner_id": owner_id, "roles": roles}
        )
    ...
    def authenticate_owner(self, owner_id: str | None, supplied_key: str | None) -> str:
        ...
        row = self.database.one(
            "SELECT api_key_hash,status FROM owners WHERE owner_id=?", (owner_id,)
        )
        supplied_hash = hashlib.sha256(supplied_key.encode()).hexdigest()
        if (
            not row
            or row["status"] != "active"
            or not hmac.compare_digest(row["api_key_hash"], supplied_hash)
        ):
            raise ControlPlaneError("Owner authentication failed")
        return owner_id
    ...
    def owner_agents(self, owner_id: str) -> list[dict[str, Any]]:
        rows = self.database.all(
            "SELECT * FROM agents WHERE owner=? ORDER BY agent_id", (owner_id,)
        )
        fields = {
            "allowed_tools",
            "allowed_actions",
    ...
    def register_agent(self, manifest: dict[str, Any], actor: str) -> dict[str, Any]:
        ...
        if missing:
            raise ControlPlaneError(
                f"Agent manifest missing: {', '.join(sorted(missing))}"
            )
        ...
        if self.settings.production:
            ...
            if manifest["owner"] != tenant:
                raise ControlPlaneError(
                    "Agent owner must match the authenticated tenant"
                )
            if not manifest["agent_id"].startswith(f"{tenant}--"):
                raise ControlPlaneError(
                    "Production agent IDs must be prefixed with '<tenant>--'"
                )
        canonical = _json(
            {key: manifest[key] for key in sorted(manifest) if key != "owner_signature"}
        )
        ...
        values = (
            ...
            manifest["risk_tier"],
            _json(manifest["allowed_tools"]),
            _json(manifest["allowed_actions"]),
            _json(manifest["allowed_data_classifications"]),
            int(manifest["max_delegation_depth"]),
            _json(manifest.get("approved_parents", [])),
            _json(manifest.get("approved_children", [])),
            manifest.get("expires_at"),
            manifest.get("review_date"),
            "pending",
            None,
            signature,
            now,
    ...
    def update_agent(
        self, agent_id: str, manifest: dict[str, Any], actor: str
    ) -> dict[str, Any]:
        ...
        if (
            ...
            or manifest.get("owner") != current["owner"]
        ):
    ...
    def set_agent_status(
        ...
        self.audit.append(
            f"agent.{status}", actor, agent_id=agent_id, payload={"status": status}
        )
        return self.agent(agent_id)
    ...
    def list_agents(self) -> list[dict[str, Any]]:
        return [
            ...
            for row in self.database.all("SELECT * FROM agents ORDER BY agent_id")
        ]
    ...
    def create_run(
        ...
        if agent["expires_at"] and datetime.fromisoformat(
            agent["expires_at"]
        ...
        if parent_run_id and not self.database.one(
            "SELECT run_id FROM runs WHERE run_id=? AND status='active'",
            (parent_run_id,),
        ):
        ...
        self.audit.append(
            ...
            agent_id=agent_id,
            run_id=run_id,
            payload={"task": task, "parent_run_id": parent_run_id},
        )
        public_run = dict(
            self.database.one("SELECT * FROM runs WHERE run_id=?", (run_id,))
        )
    ...
    def create_task(
        ...
        run = self.database.one(
            "SELECT * FROM runs WHERE run_id=? AND status='active'", (run_id,)
        )
        if not run:
        ...
        return dict(
            self.database.one("SELECT * FROM tasks WHERE task_id=?", (task_id,))
        )
    ...
    def revoke_run(self, run_id: str, actor: str, reason: str) -> None:
        ...
        with self.database.connect() as connection:
            ...
            for descendant in descendants:
                connection.execute(
                    "UPDATE runs SET status='revoked',revoked_at=? WHERE run_id=?",
                    (now, descendant),
                )
                connection.execute(
                    "UPDATE tokens SET status='revoked',revoked_at=? WHERE run_id=?",
                    (now, descendant),
                )
    ...
    def issue_capability(
        ...
        *,
        ...
        agent = self.database.one(
            "SELECT * FROM agents WHERE agent_id=?", (run["agent_id"],)
        )
    ...
    def delegate_capability(
        ...
        *,
        ...
        child_run = self.database.one(
            "SELECT * FROM runs WHERE run_id=?", (child_run_id,)
        )
        ...
        if (
            child_run["principal_id"] != parent["principal_id"]
            or child_run["parent_run_id"] != parent["run_id"]
        ):
            raise ControlPlaneError(
                "Child run is not bound to the parent principal and run"
            )
        ...
        try:
            narrower = all(
                any(
                    ...
                    for pattern in parent["resources"]
                )
        ...
        if not narrower:
            raise ControlPlaneError(
                "Child resources must be narrower than parent resources"
            )
    ...
    def register_connector(
        self, connector: dict[str, Any], actor: str
    ) -> dict[str, Any]:
        required = {
            "connector_id",
            "tool",
        ...
        if self.settings.production:
            ...
            if connector["owner"] != tenant:
                raise ControlPlaneError(
                    "Connector owner must match the authenticated tenant"
                )
            if not connector["connector_id"].startswith(f"{tenant}--"):
                raise ControlPlaneError(
                    "Production connector IDs must be prefixed with '<tenant>--'"
                )
        now = _now()
        self.database.execute(
            ...
            (
                ...
                connector.get("http_method"),
                _json(connector["resource_patterns"]),
                _json(connector["required_scopes"]),
                connector.get("secret_alias"),
                connector.get("status", "active"),
                connector["owner"],
                connector["risk_tier"],
                int(connector.get("rate_limit_per_minute", 30)),
                connector.get("credential_mode", "bearer"),
                _json(connector.get("credential_config", {})),
                int(bool(connector.get("grant_required", False))),
                now,
                now,
            ),
        )
    ...
    def list_connectors(self) -> list[dict[str, Any]]:
        return [
            self._decode(row, {"resource_patterns", "required_scopes"})
    ...
    def set_connector_status(
        self, connector_id: str, status: str, actor: str
    ) -> dict[str, Any]:
        if status not in {"active", "suspended", "retired"}:
            raise ControlPlaneError("Invalid connector status")
        self.database.execute(
            "UPDATE connectors SET status=?,updated_at=? WHERE connector_id=?",
            (status, _now(), connector_id),
        )
        self.audit.append(
            f"connector.{status}", actor, payload={"connector_id": connector_id}
        )
    ...
    def seed_policy(
        ...
        version_row = self.database.one(
            "SELECT MAX(version) AS version FROM policy_bundles WHERE policy_id=?",
            (policy_id,),
        )
    ...
    def resolve_approval(
        self, approval_id: str, approved: bool, actor: str, reason: str = ""
    ) -> dict[str, Any]:
        row = self.database.one(
    ...
    def _notify_approval(
        self, approver: str, approval_id: str, action: str, resource: str
    ) -> str:
        if "@" not in approver or not self.settings.approval_smtp_host:
            return "inbox_only"
        message = EmailMessage()
        message["From"] = self.settings.approval_smtp_from or "warden@localhost"
        message["To"] = approver
        message["Subject"] = f"Warden approval required: {action}"
        message.set_content(
            f"Approval {approval_id} requires review.\nAction: {action}\nResource: {resource}\n"
        )
        try:
            with smtplib.SMTP_SSL(
                self.settings.approval_smtp_host,
                self.settings.approval_smtp_port,
                timeout=10,
            ) as smtp:
                if self.settings.approval_smtp_username:
                    smtp.login(
                        self.settings.approval_smtp_username,
                        self.settings.approval_smtp_password or "",
                    )
            ...
            self.audit.append(
                ...
                decision="error",
                payload={"approval_id": approval_id, "channel": "email"},
            )
            return "email_failed"
        self.audit.append(
            "approval.notified",
    ...
    def approvals(
        self, status: str | None = None, approver: str | None = None
    ) -> list[dict[str, Any]]:
        clauses, parameters = [], []
        if status:
        ...
        if approver:
            ...
            parameters.append(approver)
        sql = "SELECT * FROM approvals"
        if clauses:
            sql += " WHERE " + " AND ".join(clauses)
        rows = self.database.all(sql + " ORDER BY requested_at", tuple(parameters))
        return [dict(row) for row in rows]

    def approval(self, approval_id: str, approver: str | None = None) -> dict[str, Any]:
        row = self.database.one(
            "SELECT * FROM approvals WHERE approval_id=?", (approval_id,)
        )
        if not row:
            raise ControlPlaneError("Approval not found")
        if approver and row["requested_by"] != approver:
            raise WardenAPIError("forbidden", "Approval belongs to another approver")
        return dict(row)

    # -- Mandatory action gateway --------------------------------------
    def enforcement_trace(self, call_id: str) -> dict[str, Any]:
        rows = self.database.all(
            "SELECT stage,position,status,detail,recorded_at FROM enforcement_trace_records WHERE call_id=? ORDER BY position",
            (call_id,),
        )
        if not rows:
            raise WardenAPIError("not_found", "Enforcement trace was not found")
        return {
            "call_id": call_id,
            "stages": [
                {
    ...
    def _start_trace(self, call_id: str) -> None:
        with self.database.connect() as connection:
            for position, stage in self.TRACE_STAGES:
                connection.execute(
                    "INSERT INTO enforcement_trace_records VALUES(?,?,?,?,?,NULL)",
                    (call_id, stage, position, "unrecorded", "{}"),
                )

    def _trace(
        self, call_id: str, stage: str, status: str, detail: dict[str, Any]
    ) -> None:
        self.database.execute(
            """UPDATE enforcement_trace_records SET status=?,detail=?,recorded_at=?
            WHERE call_id=? AND stage=?""",
            (status, _json(detail), _now(), call_id, stage),
        )

    def _trace_policy_layers(
        self, call_id: str, layers: tuple[str, ...], status: str, reason: str
    ) -> None:
        self._trace(
            call_id,
            "policy_layers",
            status,
            {"layers": list(layers), "reason": reason},
        )
        with self.database.connect() as connection:
            for index, layer in enumerate(layers, 1):
                connection.execute(
                    """INSERT INTO enforcement_trace_records
                    (call_id,stage,position,status,detail,recorded_at) VALUES(?,?,?,?,?,?)""",
                    (
                        call_id,
                        f"policy_layer:{layer}",
                        30.0 + index / 100,
                        status,
                        _json({"layer": layer, "reason": reason}),
                        _now(),
                    ),
                )

    def _skip_trace_after(self, call_id: str, stage: str, reason: str) -> None:
        positions = dict((name, position) for position, name in self.TRACE_STAGES)
        self.database.execute(
            """UPDATE enforcement_trace_records SET status='skipped',detail=?,recorded_at=?
            WHERE call_id=? AND position>? AND status='unrecorded'""",
            (_json({"reason": reason}), _now(), call_id, positions[stage]),
        )
    ...
    def execute_action(
        ...
        *,
        ...
        self._start_trace(tool_call_id)
        current_stage = "capability_check"
        ...
        try:
            ...
            claims = self.capabilities.verify(
                token, expected_action=action, expected_resource=resource
            )
            ...
            self._trace(
                ...
                "capability_check",
                "passed",
                {"run_id": claims["run_id"], "capability_jti": claims["jti"]},
            )
            current_stage = "grant_check"
            ...
            run = self.database.one(
                "SELECT * FROM runs WHERE run_id=?", (claims["run_id"],)
            )
            agent = self.database.one(
                "SELECT * FROM agents WHERE agent_id=?", (claims["agent_id"],)
            )
            task = self.database.one(
                "SELECT * FROM tasks WHERE task_id=? AND run_id=?",
                (task_id, claims["run_id"]),
            )
            connector = self.database.one(
                "SELECT * FROM connectors WHERE connector_id=?", (connector_id,)
            )
            ...
            if not connector or connector["action"] != action:
                raise ControlPlaneError(
                    "Connector does not implement the requested action"
                )
            ...
            self._trace(
                ...
                "grant_check",
                "passed",
                {"grant_id": grant_id, "required": bool(connector["grant_required"])},
            )
            self.database.execute(
            ...
            policy_status = (
                "passed" if decision.allowed or decision.approval_required else "failed"
            )
            self._trace_policy_layers(
                tool_call_id, decision.layers, policy_status, decision.reason
            )
            self.audit.append(
                "policy.allowed" if decision.allowed else "policy.denied",
                "policy-service",
                principal_id=claims["principal_id"],
                agent_id=claims["agent_id"],
                run_id=claims["run_id"],
                ...
                payload={
                    ...
                    "resource": resource,
                    "grant_id": grant_id,
                    "policy_layers": list(decision.layers),
                },
            )
            if not decision.allowed:
                self.database.execute(
                    "UPDATE tool_calls SET status='denied',completed_at=? WHERE tool_call_id=?",
                    (_now(), tool_call_id),
                )
                if decision.approval_required:
                    self._trace(
                        ...
                        "approval_gate",
                        "failed",
                        {"reason": decision.reason, "approval_required": True},
                    )
                    self._skip_trace_after(
                        tool_call_id, "approval_gate", "Waiting for human approval"
                    )
                ...
                self._skip_trace_after(tool_call_id, "policy_layers", decision.reason)
                return finish(
                    {
                        "status": "denied",
                        "reason": decision.reason,
            ...
            self._trace(
                ...
                "approval_gate",
                "passed",
                {"approval_id": approval_id, "required": bool(approval_id)},
            )

            current_stage = "credential_resolution"
            ...
            self._trace(
                ...
                {
                    "grant_id": grant_id,
                    "credential_resolved": downstream_secret is not None,
                },
            )
            self.audit.append(
                "connector.invoked",
                ...
                payload={
                    ...
                    "action": action,
                    "resource": resource,
                    "synthetic": connector["adapter_type"] == "local_emulator",
                },
                key_id=key_id,
            )
            current_stage = "connector_dispatch"
            result = self.connectors.execute(
                connector, resource, parameters, downstream_secret
            )
            self._trace(
                tool_call_id,
                "connector_dispatch",
            ...
            self.database.execute(
                "UPDATE tool_calls SET status='executed',completed_at=? WHERE tool_call_id=?",
                (_now(), tool_call_id),
            )
            if approval_id:
                self.database.execute(
            ...
            self._skip_trace_after(tool_call_id, current_stage, str(exc))
            if approval_id and claims and isinstance(exc, ConnectorError):
                self.database.execute(
                    ...
                    (
                        "Downstream result is uncertain; operator reconciliation required",
                        approval_id,
                    ),
                )
            self.audit.append(
                ...
                "action-gateway",
                principal_id=claims.get("principal_id") if claims else None,
                agent_id=claims.get("agent_id") if claims else None,
                run_id=claims.get("run_id") if claims else None,
                task_id=task_id,
                tool_call_id=tool_call_id,
                decision="deny",
                key_id=key_id,
            ...
            code = (
                ...
                else ("revoked" if "revok" in str(exc).lower() else "policy_denied")
            )
            return finish(
                {
                    "status": "denied"
                    ...
                    "error": {
                        "code": code,
                        "detail": str(exc),
                        "retryable": isinstance(exc, ConnectorError),
                    },
                }
            )
    ...
    def revoke_policy(self, policy_id: str, actor: str, reason: str) -> None:
        policy_id = self.database.namespace(policy_id)
        self.database.execute(
            "UPDATE policy_bundles SET status='revoked' WHERE policy_id=?", (policy_id,)
        )
    ...
    def reconcile_stale_operations(
        ...
        executions = self.database.all(
            ...
            WHERE status='processing' AND updated_at<? LIMIT 1000""",
            (cutoff,),
        )
        approvals = self.database.all(
            """SELECT approval_id FROM approvals WHERE status='executing'
            AND COALESCE(resolved_at,requested_at)<? LIMIT 1000""",
            (cutoff,),
        )
        uncertain = _json(
            {
        ...
        for row in approvals:
            self.database.execute(
                ...
                (
                    "Execution outcome requires operator reconciliation",
                    row["approval_id"],
                ),
            )
        result = {
    ...
    def bootstrap_support_demo(
        self, actor: str = "control-plane-admin"
    ) -> dict[str, Any]:
        if self.settings.production:
            raise ControlPlaneError(
                "Reference demo bootstrap is disabled in production"
            )
        agents = [
            {
                ...
                "owner": "cx-security",
                "purpose": "Triage enterprise support cases",
                ...
                "allowed_data_classifications": ["internal", "sensitive"],
                "max_delegation_depth": 1,
                "approved_parents": [],
                "approved_children": ["code-reviewer"],
            ...
            },
            {
                ...
                "purpose": "Perform read-only technical review",
                "model_provider": "local-demo",
                "agent_version": "1.0.0",
                "environment": "prod",
                "risk_tier": "medium",
                "allowed_tools": ["github"],
                "allowed_actions": ["github.read"],
                "allowed_data_classifications": ["internal"],
                "max_delegation_depth": 0,
                "approved_parents": ["support-triage"],
                "approved_children": [],
                "review_date": (datetime.now(timezone.utc) + timedelta(days=90))
                .date()
                .isoformat(),
        ...
        for manifest in agents:
            if not self.database.one(
                "SELECT agent_id FROM agents WHERE agent_id=?", (manifest["agent_id"],)
            ):
                self.register_agent(manifest, actor)
                self.approve_agent(cast(str, manifest["agent_id"]), actor)
        connectors = [
            {
                "connector_id": "crm-case-read",
                "tool": "crm",
                "action": "crm.read_case",
                "adapter_type": "local",
                "resource_patterns": ["crm://case/*"],
                "required_scopes": ["crm.read_case"],
                "owner": "cx-platform",
                "risk_tier": "low",
            },
            {
                "connector_id": "crm-case-update",
                "tool": "crm",
                "action": "crm.update_case",
                "adapter_type": "local",
                "resource_patterns": ["crm://case/*"],
                "required_scopes": ["crm.update_case"],
                "owner": "cx-platform",
                "risk_tier": "high",
            },
            {
                "connector_id": "email-draft",
                "tool": "email",
                "action": "email.draft",
                "adapter_type": "local",
                "resource_patterns": ["email://case/*"],
                "required_scopes": ["email.draft"],
                "owner": "communications",
                "risk_tier": "medium",
            },
            {
                "connector_id": "jira-create",
                "tool": "jira",
                "action": "jira.create_ticket",
                "adapter_type": "local",
                "resource_patterns": ["jira://case/*"],
                "required_scopes": ["jira.create_ticket"],
                "owner": "engineering",
                "risk_tier": "high",
            },
            {
                ...
                "tool": "github",
                "action": "github.read",
                "adapter_type": "github_readonly",
                "resource_patterns": ["github://repo/*"],
        ...
        if not self.database.one(
            "SELECT policy_id FROM policy_bundles WHERE status='active'"
        ):
            self.seed_policy({}, actor)
        if not self.database.one(
            "SELECT case_id FROM crm_cases WHERE case_id='CASE-1042'"
        ):
            self.database.execute(
                ...
                (
                    "CASE-1042",
                    "Acme Enterprise",
                    "Enterprise account is broken",
                    "new",
                    "urgent",
                    "Customer requested an immediate update.",
                    _now(),
                ),
            )
        return {
            "agents": [agent["agent_id"] for agent in agents],
            "connectors": [item["connector_id"] for item in connectors],
            "case_id": "CASE-1042",
        }

    def run_support_ticket_scenario(
        self, principal_id: str, admin_actor: str
    ) -> dict[str, Any]:
        self.bootstrap_support_demo(admin_actor)
        parent_run = self.create_run(
            principal_id,
            "support-triage",
            "Handle urgent support ticket CASE-1042",
            "prod",
        )
        parent_task = self.create_task(
            parent_run["run_id"], "Triage, update and coordinate CASE-1042"
        )
        parent_token, parent_claims = self.issue_capability(
            ...
            resources=[
                "crm://case/CASE-1042",
                "email://case/CASE-1042",
                "jira://case/CASE-1042",
                "github://repo/acme/support",
            ],
            ttl_seconds=900,
        )
        ...
        crm_request = dict(
            ...
            action="crm.update_case",
            resource="crm://case/CASE-1042",
            parameters={
                "update": "Engineering investigation started; customer update pending.",
                "status": "investigating",
            },
            data_classification="sensitive",
            environment="prod",
            risk_signals={"anomaly_score": 0.1},
        ...
        sensitive_token, _ = self.capabilities.issue(
            ...
            run_id=parent_run["run_id"],
            principal_id=principal_id,
            scopes=parent_claims["scopes"] + ["data:sensitive"],
            resources=parent_claims["resources"],
            ttl_seconds=900,
            actor="token-service",
        )
        ...
        approval = self.resolve_approval(
            pending["approval_id"],
            True,
            admin_actor,
            "Urgent customer-impacting incident",
        )
        outcomes.append(
            {
                "step": "crm_approval",
        ...
        executed = self.execute_action(
            **crm_request, approval_id=approval["approval_id"]
        )
        ...
        email = self.execute_action(
            ...
            runtime_proof=parent_run["runtime_proof"],
            action="email.draft",
            resource="email://case/CASE-1042",
            parameters={
                "recipient": "customer@example.com",
                "subject": "Update on CASE-1042",
                "body": "We are actively investigating your enterprise account issue.",
            },
        ...
        jira_request = dict(
            ...
            runtime_proof=parent_run["runtime_proof"],
            action="jira.create_ticket",
            resource="jira://case/CASE-1042",
            parameters={
                "summary": "Enterprise account failure for CASE-1042",
                "description": "Investigate the account access regression.",
            },
            data_classification="internal",
            environment="prod",
            request_nonce=str(uuid4()),
        ...
        jira_approval = self.resolve_approval(
            jira_pending["approval_id"],
            True,
            admin_actor,
            "Engineering escalation approved",
        )
        jira_request["request_nonce"] = str(uuid4())
        jira = self.execute_action(
            **jira_request, approval_id=jira_approval["approval_id"]
        )
        outcomes.append({"step": "jira_created", **jira})
        child_run = self.create_run(
            principal_id,
            "code-reviewer",
            "Review technical evidence for CASE-1042",
            "prod",
            parent_run["run_id"],
        )
        child_task = self.create_task(
            child_run["run_id"],
            "Read-only review of support repository",
            parent_task["task_id"],
        )
        child_token, child_claims = self.delegate_capability(
            parent_token=parent_token,
            parent_runtime_proof=parent_run["runtime_proof"],
            child_run_id=child_run["run_id"],
            scopes=["github.read"],
            resources=["github://repo/acme/support"],
            ttl_seconds=300,
        )
        review = self.execute_action(
            ...
            action="github.read",
            resource="github://repo/acme/support",
            parameters={"reference": "main"},
            data_classification="internal",
            environment="prod",
            request_nonce=str(uuid4()),
        )
        outcomes.append(
            {"step": "child_code_review", **review, "child_jti": child_claims["jti"]}
        )
        verification = self.audit.verify()
        return {
            "status": "completed"
            if all(
                item["status"] in {"approval_required", "approved", "executed"}
            ...
            "authority": {
                ...
                "child_jti": child_claims["jti"],
                "child_scopes": child_claims["scopes"],
            },
            "audit": verification,
        }
    ...
    def _request_approval(
        self, claims: dict[str, Any], task_id: str, action: str, resource: str
    ) -> dict[str, Any]:
        ...
        notification = self._notify_approval(
            claims["principal_id"], approval_id, action, resource
        )
        result = dict(
            self.database.one(
                "SELECT * FROM approvals WHERE approval_id=?", (approval_id,)
    ...
    def _verify_runtime_proof(self, run_id: str, supplied: str) -> None:
        row = self.database.one(
            "SELECT runtime_secret_hash FROM runs WHERE run_id=?", (run_id,)
        )
        supplied_hash = hashlib.sha256((supplied or "").encode()).hexdigest()
        if (
            not row
            or not row["runtime_secret_hash"]
            or not hmac.compare_digest(row["runtime_secret_hash"], supplied_hash)
        ):
            raise ControlPlaneError("Runtime proof mismatch")

    def _revocation(
        self, target_type: str, target_id: str, actor: str, reason: str
    ) -> None:
        self.database.execute(
    ...
    def _ensure_defaults(self) -> None:
        ...
        if not self.database.one("SELECT key FROM settings WHERE key=?", (key,)):
            self.database.execute(
                "INSERT INTO settings VALUES(?,?,?)", (key, "false", _now())
            )
